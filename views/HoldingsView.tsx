...
const HoldingsView: React.FC<Props> = ({ data, onUpdate, globalYear }) => {
  ...
  const closedPositions = allPositions.filter(p => p.qty === 0 && p.real !== 0).sort((a, b) => b.real - a.real);
             ...
             <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50">
                   {activePositions.map((pos) => {
                      ...
                      return (
                      <tr key={pos.symbol} className="hover:bg-gray-50/80 transition-colors">
                   ...
                   {activePositions.length === 0 && (
                     <tr><td colSpan={6} className="px-8 py-20 text-center text-gray-300 font-bold italic uppercase text-xs">Keine aktiven Positionen im Jahr {currentYear}</td></tr>
                   )}
                </tbody>
             </table>
          </div>
