...
import { Package, Download, Archive, RefreshCw, AlertCircle, CheckCircle, FolderOpen } from 'lucide-react';
  ...
  const handleExportAll = async () => {
    ...
    try {
      ...
      if (VaultService.isConnected()) {
          ...
          setMessage({ type: 'success', text: `Vollständiges Backup "${filename}" direkt im Vault-Ordner gespeichert!` });
      } else {
          ...
          setMessage({ type: 'success', text: "Backup ZIP erfolgreich erstellt und heruntergeladen." });
      }
    } catch (err: any) {
      console.error(err);
