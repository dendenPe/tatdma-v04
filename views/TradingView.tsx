    ...
    reader.onload = (event) => {
      ...
      Object.entries(importedDays).forEach(([date, importedEntry]) => {
          if (!newData.trades[date]) {
              ...
              // Merge if exists
              newData.trades[date].trades = [...newData.trades[date].trades, ...importedEntry.trades];
              newData.trades[date].fees = (newData.trades[date].fees || 0) + (importedEntry.fees || 0);
              // Recalculate total
              const allTrades = newData.trades[date].trades;
              const gross = allTrades.reduce((s, t) => s + t.pnl, 0);
              const fees = newData.trades[date].fees || 0;
              newData.trades[date].total = gross - fees;
          }
      ...
      alert(`${count} Trades erfolgreich importiert! Gebühren wurden erfasst.`);
    };
